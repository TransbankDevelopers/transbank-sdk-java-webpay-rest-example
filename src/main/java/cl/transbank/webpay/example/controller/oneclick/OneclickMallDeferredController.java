...
public class OneclickMallDeferredController extends BaseController {
  ...
  public ModelAndView start(HttpServletRequest request) {
    ...
    try {
      final OneclickMallInscriptionStartResponse response = inscription.start(
        username,
        email,
        returnUrl
      );
  ...
  public ModelAndView authorize(
    ...
    String childCommerceCode1 =
      IntegrationCommerceCodes.ONECLICK_MALL_DEFERRED_CHILD1;
    String childCommerceCode2 =
      IntegrationCommerceCodes.ONECLICK_MALL_DEFERRED_CHILD2;

    MallTransactionCreateDetails mallDetails = MallTransactionCreateDetails
      .build()
      .add(
        childAmount1,
        childCommerceCode1,
    ...
    try {
      final OneclickMallTransactionAuthorizeResponse response = tx.authorize(
        username,
        tbkUser,
        buyOrder,
        mallDetails
      );
      OneclickMallTransactionAuthorizeResponse.Detail detail = response
        .getDetails()
        .get(0);
    ...
    return new ModelAndView(
      "oneclick_mall_deferred/authorize",
      "details",
      details
    );
  }
  ...
  public ModelAndView refund(
    ...
    try {
      final OneclickMallTransactionRefundResponse response = tx.refund(
        buyOrder,
        childCommerceCode,
        childBuyOrder,
        amount
  ...
  public ModelAndView status(@RequestParam("buy_order") String buyOrder) {
    Map<String, Object> details = new HashMap<>();
    try {
      final OneclickMallTransactionStatusResponse response = tx.status(
        buyOrder
      );
      details.put("response", response);
      details.put("resp", toJson(response));
  ...
  public ModelAndView capture(
    ...
    Map<String, Object> details = new HashMap<>();
    try {
      final OneclickMallTransactionCaptureResponse response = tx.capture(
        childCommerceCode,
        childBuyOrder,
        authorizationCode,
        amount
    ...
    return new ModelAndView(
      "oneclick_mall_deferred/capture",
      "details",
      details
    );
  }
